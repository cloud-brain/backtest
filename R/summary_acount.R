...
summary_acount <- function(total_acount, benchmark = F, ylog = T, plot = T, show_result = T, ...)
{
  ...
  if(benchmark)
  {
    ...
    if(plot) PerformanceSummary_rel(as.xts(yield %>% select(-date), order.by = yield$date), ylog = ylog, ...)
    output <- yield %>% 
    ...
    if(plot) charts.PerformanceSummary(as.xts(yield %>% select(-date), order.by = yield$date), ylog = ylog, ...)
    output <- yield %>% 
      group_by(year = format(date, '%Y')) %>%
      summarise(yield = prod(1 + acount) - 1,
                max_back = max_back(1 + acount),
